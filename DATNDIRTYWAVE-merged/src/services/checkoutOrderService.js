...
const flattenVariants = (products = []) => {
  return products.flatMap((product) => {
    if (!isActiveProductStatus(product?.trangThai || product?.status)) return []
...
const resolveVariantForItem = (item, variants) => {
  ...
  // Customer checkout must always carry a concrete product reference.
  ...
  const scopedVariants = productId > 0
    ? variants.filter((variant) => Number(variant.productId) === productId)
    : variants
  ...
  if (productId > 0) {
    ...
    if (normalizedColor || normalizedSize) {
      const exactBySelection = scopedVariants.find((variant) => {
        const sameColor = !normalizedColor || normalizeText(variant.colorName) === normalizedColor
        const sameSize = !normalizedSize || normalizeText(variant.sizeName) === normalizedSize
        return sameColor && sameSize
      })
      if (exactBySelection) return exactBySelection
      return null
    }

    return scopedVariants[0] || null
  }
...
export async function createBackendCheckoutOrder({ cartItems, delivery, shipping = 0, discount = 0, total = null, paymentMethod = "VNPAY", statusNote = "", voucherCode = "", orderType = "ONLINE", orderStatusCode = "" }) {
  ...
  let soldBySpctMap = await computeSoldBySpctDirect().catch(() => new Map())
  if (!(soldBySpctMap instanceof Map) || soldBySpctMap.size === 0) {
    soldBySpctMap = await computeSoldBySpct().catch(() => new Map())
  }
  ...
  if (unresolvedItems.length) {
    throw new Error(`Một số sản phẩm không còn kinh doanh hoặc biến thể đã thay đổi: ${unresolvedItems.map((item) => item.name).join(", ")}`)
  }

  const invalidStockItems = resolvedItems.filter((item) => {
    const variantRaw = item?.variant?.raw || {}
    const inStock = variantAvailableStock(variantRaw, soldBySpctMap)
