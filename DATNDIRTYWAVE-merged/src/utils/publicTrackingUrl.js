...
export const buildOrderLookupTrackingUrl = ({ maHoaDon }) => {
  ...
  const runtimeOrigin = normalizeOriginCandidate(typeof window !== 'undefined' ? window.location.origin : '')
  const appOrigin = publicOrigin || runtimeOrigin

  if (!appOrigin) {
    return `/tra-cuu-don-hang?${new URLSearchParams({ ma }).toString()}`
  }

  return `${appOrigin}/tra-cuu-don-hang?${new URLSearchParams({ ma }).toString()}`
}