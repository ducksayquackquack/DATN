...
const checkoutAvailabilityNoticeKey = ref("")
const checkoutAvailabilityLastNotifyAt = ref(0)
const CHECKOUT_AVAILABILITY_NOTIFY_COOLDOWN_MS = 60000
...
const isActiveProductStatus = (value = "") => {
  ...
  return !normalized.includes("ngung")
    ...
    && !normalized.includes("khong kinh doanh")
...
const resolveExactVariantForItem = (item) => {
  ...
  // If user has explicit variant selectors but no exact match, treat as unavailable.
  const hasExplicitSelection = Boolean(item?.spctId || item?.color || item?.size)
...
const getAvailableStockForItem = (item) => {
  const exact = resolveExactVariantForItem(item)
  if (!exact) return null

  return getVariantStock(exact)
}

const isItemUnavailable = (item) => {
  const variants = getVariantsForItem(item)
  if (!variants.length) return true
  if (item?._variantMissing) return true
  if (!resolveExactVariantForItem(item)) return true
...
const pruneUnavailableCartItems = (items = []) => {
  if (!Array.isArray(items) || !items.length) return 0

  const removedKeys = new Set(items.map((item) => getItemKey(item)))
  const beforeSize = cart.value.length
  cart.value = cart.value.filter((entry) => !removedKeys.has(getItemKey(entry)))

  if (beforeSize !== cart.value.length) {
    const nextSelections = { ...itemVoucherSelections.value }
    for (const key of Object.keys(nextSelections)) {
      if (removedKeys.has(key)) delete nextSelections[key]
    }
    itemVoucherSelections.value = nextSelections
    persistCheckoutCart()
  }

  return beforeSize - cart.value.length
}
...
const syncItemVariant = (item) => {
  const variants = getVariantsForItem(item)
  if (!variants.length) {
    item._variantMissing = true
    return
  }

  let matched = variants.find((variant) => Number(variant?.spctId || 0) === Number(item?.spctId || 0))

  const hasExplicitSelection = Boolean(item?.spctId || item?.color || item?.size)
  if (!matched) {
    matched = variants.find((variant) => variant.color === item.color && variant.size === item.size)
  }

  if (!matched && !hasExplicitSelection) {
    matched = variants[0]
  }
...
const loadVariantCatalog = async () => {
  try {
    ...
    for (const product of products) {
      ...
      if (!isActiveProductStatus(product?.trangThai || product?.status)) {
        catalog[productId] = []
        continue
      }
      ...
      catalog[productId] = pricedVariants.map((variant) => {
        ...
        return {
          ...
          image: varImg,
          availableStock: variantAvailableStock(variant, soldBySpct.value)
        }
      })
    }
...
const refreshCheckoutAvailability = async ({ notify = false } = {}) => {
  await loadVariantCatalog()

  const unavailableItems = cart.value.filter((item) => isItemUnavailable(item))
  const exceededItems = cart.value.filter((item) => {
    const availableStock = getAvailableStockForItem(item)
    return Number.isFinite(availableStock) && availableStock >= 0 && Number(item?.quantity || 0) > availableStock
  })

  for (const item of exceededItems) {
    const availableStock = getAvailableStockForItem(item)
    if (Number.isFinite(availableStock) && availableStock >= 0) {
      item.quantity = Math.max(1, Math.floor(availableStock))
    }
  }

  const removedCount = pruneUnavailableCartItems(unavailableItems)

  const unavailableNames = unavailableItems.map((item) => String(item?.name || "Sản phẩm").trim()).filter(Boolean).join("|")
  const exceededNames = exceededItems.map((item) => String(item?.name || "Sản phẩm").trim()).filter(Boolean).join("|")
  const nextNoticeKey = `${unavailableNames}::${exceededNames}::${removedCount}`

  if (notify) {
    const hasNotice = unavailableItems.length > 0 || exceededItems.length > 0
    const now = Date.now()
    const cooldownPassed = now - Number(checkoutAvailabilityLastNotifyAt.value || 0) >= CHECKOUT_AVAILABILITY_NOTIFY_COOLDOWN_MS

    if (hasNotice && checkoutAvailabilityNoticeKey.value !== nextNoticeKey && cooldownPassed) {
      checkoutAvailabilityNoticeKey.value = nextNoticeKey
      checkoutAvailabilityLastNotifyAt.value = now

      if (unavailableItems.length) {
        const removedNames = unavailableItems.map((item) => item?.name || "Sản phẩm").join(", ")
        toast.error(`Đã bỏ khỏi giỏ vì hết hàng/ngừng kinh doanh: ${removedNames}`)
      }

      if (exceededItems.length) {
        toast.warning(`Đã tự điều chỉnh số lượng theo tồn kho mới. Vui lòng kiểm tra lại trước khi thanh toán.`)
      }
    }
  }

  return {
    ok: unavailableItems.length === 0 && exceededItems.length === 0,
    unavailableItems,
    exceededItems
  }
}
...
const handleCheckoutVisibilityRefresh = () => {
  if (typeof document !== "undefined" && document.visibilityState === "hidden") {
    return
  }
  refreshCheckoutAvailability({ notify: false }).catch(() => {})
}
...
const validateCheckout = () => {
  ...
  const unavailableItem = cart.value.find((item) => isItemUnavailable(item))
  if (unavailableItem) {
    return `Sản phẩm ${unavailableItem?.name || ""} không còn kinh doanh hoặc đã hết hàng`
  }
...
const submitOrder = async () => {
  const refreshed = await refreshCheckoutAvailability({ notify: true })
  if (!refreshed.ok) {
    return
  }
...
onMounted(async () => {
  ...
  await refreshCheckoutAvailability()
...
onMounted(() => {
  window.addEventListener("focus", handleCheckoutVisibilityRefresh)
  document.addEventListener("visibilitychange", handleCheckoutVisibilityRefresh)
})

onBeforeUnmount(() => {
  window.removeEventListener("focus", handleCheckoutVisibilityRefresh)
  document.removeEventListener("visibilitychange", handleCheckoutVisibilityRefresh)
})
