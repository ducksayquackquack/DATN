...
const getCurrentCustomer = async (accountId) => {
  try {
    const byTaiKhoan = await getKhachHangByTaiKhoanId(accountId)
    if (byTaiKhoan?.data?.id && getTaiKhoanIdFromKhachHang(byTaiKhoan.data) === Number(accountId)) {
      return byTaiKhoan.data
    }
  } catch {
    // Fallback below.
  }
  ...
  // Do not fallback by email to avoid linking wrong customer profile.
  return null
}

const doesOrderBelongToCustomer = (item, customerId, accountId, accountEmail = '') => {
  ...
  const orderCustomerId = Number(item?.khachHang?.id || item?.khachHangId || item?.idKhachHang || 0)
  if (!Number.isFinite(orderCustomerId) || orderCustomerId <= 0 || orderCustomerId !== normalizedCustomerId) {
    return false
  }

  const orderAccountId = Number(
    item?.khachHang?.idTaiKhoan
    || item?.khachHang?.taiKhoan?.id
    || item?.taiKhoanId
    || item?.idTaiKhoan
    || 0
  )
  if (!Number.isFinite(orderAccountId) || orderAccountId <= 0 || orderAccountId !== normalizedAccountId) {
    return false
  }

  const orderEmail = String(
    item?.khachHang?.taiKhoan?.email
    || item?.khachHang?.email
    || item?.customerEmail
    || ''
  ).trim().toLowerCase()
  if (!orderEmail || orderEmail !== normalizedAccountEmail) {
    return false
  }
...
const loadOrders = async (customerId, accountRef = null) => {
  const offlineOrders = mapOfflineOrders(customerId)
  const accountId = Number(accountRef?.id || account.value?.id || 0)
  const accountEmail = String(accountRef?.email || account.value?.email || '').trim().toLowerCase()
  ...
  try {
    ...
    const backendOrders = list
      .filter((item) => doesOrderBelongToCustomer(item, customerId, accountId, accountEmail))
      .map((item) => ({
        ...item,
...
const loadAccountCenter = async () => {
  ...
  try {
    ...
    customer.value = await getCurrentCustomer(account.value.id)
    fillProfileForm()

    if (customer.value?.id) {
      await ensureRealCustomerName()
      await Promise.all([
        loadOrders(customer.value.id, account.value),
