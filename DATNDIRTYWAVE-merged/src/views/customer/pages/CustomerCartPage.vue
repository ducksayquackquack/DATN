...
const resolveLineStock = (line) => {
  ...
  const calcAvailable = (variant) => {
    const raw = Number(variant?.stock || 0)
    const sold = soldBySpct.value.get(Number(variant?.spctId)) || 0
    return Math.max(0, raw - sold)
  }

  const matchedBySpct = line.variants.find((variant) => Number(variant?.spctId) === Number(line.spctId))
  if (matchedBySpct) return calcAvailable(matchedBySpct)
  ...
  if (matchedByColorSize) return calcAvailable(matchedByColorSize)
...
onMounted(() => {
  ...
  computeSoldBySpct().then((map) => { soldBySpct.value = map }).catch(() => {})
