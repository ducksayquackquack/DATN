...
const hasGatewayResult = computed(() => {
  return Boolean(responseCode.value || transactionStatus.value || txnRef.value)
})
const isSuccess = computed(() => {
  const successResponseCodes = new Set(["00", "0"])
  const successTxnStatuses = new Set(["00", "0"])

  if (!hasGatewayResult.value) return false

  const hasResponseCode = Boolean(responseCode.value)
  const hasTxnStatus = Boolean(transactionStatus.value)
  const responseOk = successResponseCodes.has(responseCode.value)
  const txnOk = successTxnStatuses.has(transactionStatus.value)

  if (hasResponseCode && hasTxnStatus) {
    return responseOk && txnOk
  }
...
const displaySuccess = computed(() => isSuccess.value || isAlreadyConfirmed.value)
const isFailed = computed(() => hasGatewayResult.value && !displaySuccess.value)
...
<template>
  <div class="result-wrap">
    <div class="result-card">
      <h1 :class="displaySuccess ? 'success' : (isFailed ? 'fail' : '')">
        {{ displaySuccess ? "Thanh toán VNPay thành công" : (isFailed ? "Thanh toán VNPay thất bại" : "Đang xác thực kết quả VNPay") }}
      </h1>

      <p class="meta">Mã phản hồi: <b>{{ responseCode || "N/A" }}</b></p>
      <p class="meta">Trạng thái giao dịch: <b>{{ transactionStatus || "N/A" }}</b></p>

      <p v-if="displaySuccess && !isAlreadyConfirmed" class="hint">
      ...
      <p v-else-if="displaySuccess && isAlreadyConfirmed" class="hint success-text">
      ...
      <p v-else-if="!hasGatewayResult" class="hint">
        Chưa nhận được mã phản hồi đầy đủ từ cổng thanh toán. Nếu bạn đã thanh toán thành công, hãy bấm <b>Xác nhận thanh toán</b> để gửi yêu cầu kiểm tra giao dịch.
      </p>
      ...
      <div class="actions">
        <button
          v-if="displaySuccess || !hasGatewayResult"
          class="btn success"
          :disabled="confirming || isAlreadyConfirmed || !hasOrderContext || isFailed"
