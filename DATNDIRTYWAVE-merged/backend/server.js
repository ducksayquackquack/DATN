...
app.post("/api/vnpay/create-payment", (req, res) => {
  ...
  const clientReturnUrlRaw = String(req.body?.returnUrl || "").trim()
  const resolvedReturnUrl = (() => {
    if (clientReturnUrlRaw) {
      try {
        const parsed = new URL(clientReturnUrlRaw)
        if (parsed.protocol === "http:" || parsed.protocol === "https:") {
          return clientReturnUrlRaw
        }
      } catch {
        // ignore invalid client returnUrl and use default fallback
      }
    }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Tra cứu đơn hàng DirtyWave</title>
</head>
<body style="margin:0;padding:24px 0;background:#f1f5f9;font-family:'Segoe UI',Arial,sans-serif;color:#111827">
  <div style="max-width:560px;margin:0 auto;border-radius:14px;overflow:hidden;border:1px solid #e2e8f0;box-shadow:0 8px 26px rgba(15,23,42,.08)">
    <div style="background:linear-gradient(140deg,#111827 0%,#b91c1c 45%,#dc2626 100%);padding:28px 24px;text-align:center">
      <h1 style="margin:0 0 6px;font-size:24px;line-height:1.2;color:#fff;letter-spacing:.02em">TRA CỨU ĐƠN HÀNG DIRTYWAVE</h1>
      <p style="margin:0;font-size:13px;color:rgba(255,255,255,.88)">Mã đơn hàng của bạn: <b>${maHoaDon}</b></p>
    </div>

    <div style="background:#fff;padding:24px">
      <p style="margin:0 0 12px;font-size:14px;color:#334155;line-height:1.6">Bạn có thể bấm nút bên dưới để theo dõi trạng thái đơn hàng mới nhất.</p>
      <a href="${safeTrackingUrl}" style="display:inline-block;padding:12px 18px;border-radius:10px;background:#dc2626;color:#fff;text-decoration:none;font-weight:700">Tra cứu đơn hàng</a>
      <p style="margin:14px 0 0;font-size:13px;color:#64748b;line-height:1.6">Nếu nút không hoạt động, hãy sao chép đường dẫn sau và mở trên trình duyệt:</p>
      <p style="margin:6px 0 0;word-break:break-all;font-size:12px;color:#0f172a">${safeTrackingUrl}</p>
      ${soDienThoai ? `<p style="margin:12px 0 0;font-size:12px;color:#64748b">SĐT nhận hàng: ${soDienThoai}</p>` : ""}
    </div>
