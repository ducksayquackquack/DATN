...
const currentProduct = computed(() => {
  return PRODUCTS.find((item) => Number(item.id) === Number(route.params.id)) || null
})

// Sản phẩm hiển thị: ưu tiên backend, fallback về PRODUCTS hardcode, cuối cùng mới dùng PRODUCTS[0]
const displayProduct = computed(() => {
  return currentProduct.value || PRODUCTS[0]
})
...
const displayImages = computed(() => {
  // Ưu tiên ảnh từ backend (colorImages của matchedBackendProduct)
  if (matchedBackendProduct.value) {
    const variants = Array.isArray(matchedBackendProduct.value.sanPhamChiTiets)
      ? matchedBackendProduct.value.sanPhamChiTiets
      : []

    // Lấy ảnh theo màu đang chọn
    const variantImages = variants
      .filter(v => {
        const colorName = v?.mauSac?.tenMau || ""
        return !selectedColor.value || colorName === selectedColor.value
      })
      .map(v => v?.hinhAnh)
      .filter(Boolean)
    ...
    // Nếu màu đang chọn tồn tại trong backend nhưng không có ảnh → không hiển thị gì
    const colorExistsInBackend = variants.some(v => v?.mauSac?.tenMau === selectedColor.value)
    if (colorExistsInBackend) return []

    // Màu không có trong backend → fallback tất cả ảnh của sản phẩm
    ...
    if (allVariantImages.length) {
      return [...new Set(allVariantImages)]
    }
  }

  // Fallback: ảnh từ PRODUCTS hardcode
  const baseImages = Array.isArray(displayProduct.value.images)
    ? displayProduct.value.images.filter(Boolean)
    : []
...
const matchedBackendProduct = computed(() => {
  // Ưu tiên tìm theo id từ URL (bao gồm cả sản phẩm mới tạo từ backend)
  const routeId = Number(route.params.id)
  const byRouteId = backendProducts.value.find((item) => Number(item?.id) === routeId)
  if (byRouteId) return byRouteId

  // Fallback: tìm theo id của PRODUCTS hardcode
...
const displayedProductCode = computed(() => {
  return matchedBackendProduct.value?.maSanPham || displayProduct.value?.sku
})

const effectiveName = computed(() => {
  return matchedBackendProduct.value?.tenSanPham || displayProduct.value?.name || ""
})

const effectiveDescription = computed(() => {
  return matchedBackendProduct.value?.moTa || displayProduct.value?.description?.intro || ""
})
...
const colorHexByName = (name) => {
  ...
  if (normalized.includes("do") || normalized.includes("đo")) return "#ef4444"
  if (normalized.includes("hong")) return "#f472b6"
  if (normalized.includes("tim")) return "#7c3aed"
  if (normalized.includes("vang")) return "#eab308"
  if (normalized.includes("cam")) return "#f97316"
  if (normalized.includes("xanh la") || normalized.includes("xanh_la") || normalized.includes("xanhla")) return "#22c55e"
  if (normalized.includes("navy") || normalized.includes("xanh navy")) return "#1e3a5f"
  if (normalized.includes("xanh")) return "#2f4f75"
  if (normalized.includes("reu")) return "#4a5845"
  if (normalized.includes("bac") || normalized.includes("bạc")) return "#c0c0c0"
  if (normalized.includes("vang dong") || normalized.includes("dong")) return "#b8860b"
  return "#9ca3af"
}
...
const effectiveColors = computed(() => {
  ...
  return (displayProduct.value?.colors || []).map((color) => ({
    name: color?.name || "",
...
const effectiveSizes = computed(() => {
  ...
  return displayProduct.value?.sizes || []
})
...
const effectivePrice = computed(() => {
  ...
  return Number(displayProduct.value?.price || 0)
})

const effectiveOriginalPrice = computed(() => {
  const oldPrice = Number(displayProduct.value?.originalPrice || 0)
...
const relatedProducts = computed(() => {
  ...
  const category = displayProduct.value?.category || ""
  const sameCategory = PRODUCTS.filter((item) => item.id !== currentId && item.category === category)
  const otherProducts = PRODUCTS.filter((item) => item.id !== currentId && item.category !== category)
...
const addToCart = () => {
  ...
  const key = String(matchedBackendProduct.value?.id || displayProduct.value?.id || route.params.id)
...
const buyNow = () => {
  ...
  localStorage.setItem("checkoutCart", JSON.stringify([{
    id: matchedBackendProduct.value?.id || displayProduct.value?.id,
    ...
    image: activeImage.value || displayProduct.value?.images?.[0]
  }]))
...
watch(matchedBackendProduct, (newVal, oldVal) => {
  // Khi backend data load xong và match được sản phẩm, sync lại màu/size/ảnh
...
watch(selectedColor, () => {
  ...
  const newImages = displayImages.value
  if (newImages.length) {
    ...
    <main class="pd-shell">
      ...
      <section class="pd-main">
        <div class="pd-gallery" :class="{ 'pd-gallery--single': displayImages.length <= 1 }">
          <div class="pd-gallery__thumbs">
            ...
            >
              <img :src="image" :alt="effectiveName" />
            </button>
          </div>

          <div class="pd-gallery__stage">
            <template v-if="displayImages.length > 0">
              <img :src="activeImage || displayImages[0]" :alt="effectiveName" />
            ...
            <div v-else class="pd-gallery__empty">
              <span>Chưa có ảnh cho màu này</span>
            </div>
          </div>
        </div>
        ...
        <div class="pd-info">
          <div class="pd-info__panel">
            <div class="pd-info__topline">
              <h1>{{ effectiveName }}</h1>
              <span v-if="displayProduct.badge" class="pd-stock" :class="`is-${displayProduct.badgeTone}`">{{ displayProduct.badge }}</span>
            </div>
      ...
      <section class="pd-tabs">
        ...
        <div v-if="activeTab === 'description'" class="pd-tabs__panel">
          <h2>{{ effectiveName }}</h2>
          <ul v-if="displayProduct.material || displayProduct.fit" class="pd-bullets">
            <li v-if="displayProduct.material"><strong>Chất liệu:</strong> {{ displayProduct.material }}</li>
            <li v-if="displayProduct.fit"><strong>Form:</strong> {{ displayProduct.fit }}</li>
          </ul>
          <p>{{ effectiveDescription }}</p>
          <template v-if="displayProduct.description">
            <h3 v-if="displayProduct.description.material">Chất liệu</h3>
            <p v-if="displayProduct.description.material">{{ displayProduct.description.material }}</p>
            <h3 v-if="displayProduct.description.design">Kỹ thuật thiết kế</h3>
            <p v-if="displayProduct.description.design">{{ displayProduct.description.design }}</p>
            <h3 v-if="displayProduct.description.fit">Form dáng</h3>
            <p v-if="displayProduct.description.fit">{{ displayProduct.description.fit }}</p>
          </template>
        </div>
