...
const loadProducts = async () => {
  ...
  try {
    ...
    products.value = source
      .filter(item => {
        const status = String(item?.trangThai || "").trim().toLowerCase()
        if (status !== "hoạt động" && status !== "hoat dong" && status !== "active") return false
        // Ẩn nếu tất cả biến thể đều ngừng hoạt động
        const variants = Array.isArray(item?.sanPhamChiTiets) ? item.sanPhamChiTiets : []
        if (variants.length > 0) {
          const hasActive = variants.some(v => String(v?.trangThai || "").trim().toLowerCase() !== "ngừng hoạt động")
