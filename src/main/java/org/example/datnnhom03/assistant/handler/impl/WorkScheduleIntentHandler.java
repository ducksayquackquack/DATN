...
@Component
public class WorkScheduleIntentHandler extends AbstractToolIntentHandler implements IntentHandler {
    public WorkScheduleIntentHandler(ToolDispatcher toolDispatcher) { super(toolDispatcher); }
    @Override public AssistantIntent supportedIntent() { return AssistantIntent.WORK_SCHEDULE; }
    ...
    public AssistantHandlerResult handle(AssistantRequestContext context) {
        ...
        Map<String, Object> data = call(context, "get_nearest_work_schedule", Map.of(), result);
        result.setGrounded(((Number) data.getOrDefault("count", 0)).intValue() > 0 || data.containsKey("summary"));
        result.setReply(stringValue(data.getOrDefault("summary", "Em đã lấy lịch làm gần nhất.")));
        return result;
    }
}
