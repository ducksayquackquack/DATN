...
//
    ...
    private static final Set<String> SUPPORTED_BANK_CODES =
            new HashSet<>(Arrays.asList("VNPAYQR", "VNBANK", "INTCARD"));
    ...
    public static String createPaymentUrl(
        ...
        String safeTxnRef = (txnRef == null || txnRef.trim().isEmpty())
                ? String.valueOf(System.currentTimeMillis())
                : txnRef.trim();

        String safeIp = (ipAddr == null || ipAddr.trim().isEmpty())
                ? "127.0.0.1"
                : ipAddr.trim();

        String safeReturnUrl = (returnUrl == null || returnUrl.trim().isEmpty())
        ...
        params.put("vnp_CurrCode","VND");
                params.put("vnp_TxnRef",safeTxnRef);
        params.put("vnp_OrderInfo","Thanh toan don hang");
        params.put("vnp_OrderType","other");
        params.put("vnp_Locale","vn");
                params.put("vnp_ReturnUrl",safeReturnUrl);
                params.put("vnp_IpAddr",safeIp);
        params.put("vnp_CreateDate",createDate);

                if (SUPPORTED_BANK_CODES.contains(normalizedBankCode)) {
                        params.put("vnp_BankCode", normalizedBankCode);
                }
