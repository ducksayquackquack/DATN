...
const allProducts = [
  {id:1,name:"Áo Bomber Da Lộn DirtyWave",cat:"Áo khoác",price:649000,old:799000,tag:"AIRFLEX",img:img1},
  {id:2,name:"Áo Bomber Dáng Lửng",cat:"Áo khoác",price:399000,old:null,tag:"New",img:img2},
  {id:3,name:"Áo Bomber Giả Da DirtyWave",cat:"Áo khoác",price:329000,old:389000,tag:"Best",img:img3},
  {id:4,name:"Áo Bomber Cotton DirtyWave",cat:"Áo khoác",price:459000,old:null,tag:"Office",img:img4},
  ...
  {id:8,name:"Coach Cách Nhiệt Timberland",cat:"Áo khoác",price:799000,old:899000,tag:"Drop",img:img8},
  {id:9,name:"Coach Da ASOS DirtyWave",cat:"Áo khoác",price:899000,old:null,tag:"Premium",img:img9},
  {id:10,name:"Coach Giả Da DirtyWave",cat:"Áo khoác",price:699000,old:null,tag:"Trend",img:img10},
  {id:11,name:"Coach Lông Cừu DirtyWave",cat:"Áo khoác",price:399000,old:559000,tag:"399K",img:img11},
  {id:12,name:"Bomber Dáng Lửng DirtyWave",cat:"Áo khoác",price:299000,old:459000,tag:"299K",img:img2},
  {id:13,name:"Bomber Giả Da DirtyWave",cat:"Áo khoác",price:399000,old:559000,tag:"399K",img:img3},
  {id:14,name:"Bomber Vải Cotton DirtyWave",cat:"Áo khoác",price:299000,old:429000,tag:"299K",img:img4},
...
<div class="topbar">
  <div class="row-full">
    <div class="marquee">
      <span style="color:var(--muted)">🔥 CYBER SALE 🔥 Giảm đến 40% • Freeship từ 299K • Đổi size dễ • Hỗ trợ 24/7</span>
    </div>
  </div>
</div>
...
<a class="tile"
href="#best"
@click.prevent="filterBy('Áo khoác')">
<div class="tile-showcase">
  <img :src="img1" alt="Áo khoác" class="tile-img">
</div>
<div>
<h3>Áo Khoác</h3>
<span>Bomber • Coach • Hoodie</span>
</div>
</a>

<a class="tile"
href="#best"
@click.prevent="filterBy('Hoodie')">
<div class="tile-showcase">
  <img :src="img5" alt="Hoodie" class="tile-img">
</div>
<div>
<h3>Hoodie</h3>
<span>Dáng hộp • In hình • Jacket</span>
</div>
</a>

<a class="tile"
href="#best"
@click.prevent="filterBy('Jeans')">
<div class="tile-showcase">
  <img :src="img17" alt="Jeans" class="tile-img">
</div>
<div>
<h3>Jeans</h3>
<span>Slim Fit • Co giãn • Thoáng</span>
</div>
</a>
...
<button class="btn"
@click="clearFilter()">Tất cả</button>

<button class="btn"
@click="filterBy('Áo khoác')">Áo khoác</button>

<button class="btn"
@click="filterBy('Hoodie')">Hoodie</button>

<button class="btn"
@click="filterBy('Jeans')">Jeans</button>
</div>
</div>
...
<section id="new">
  <div class="container">
    <div class="section-head">
      <div>
        <h2>Hàng Mới Về</h2>
        <p>Cập nhật hằng tuần, mẫu mã mới & độc quyền</p>
      </div>
    </div>

    <div class="cards">
      <article v-for="p in products.slice(0, 4)" :key="'new-' + p.id" class="card">
        <div class="thumb">
          <img :src="p.img" alt="" />
          <div class="overlay">
            <button class="overlay-btn" @click="$router.push('/product/' + p.id)">
        ...
        <div class="footer">
          <span class="chip">{{ p.tag }}</span>
          <button class="btn" @click="addToCart(p.id)">Thêm giỏ</button>
        </div>
      </article>
    </div>
  </div>
...
<section id="outlet">
  <div class="container">
    <div class="section-head">
      <div>
        <h2>Flash Sale / Outlet</h2>
        <p>Giảm sâu • Số lượng có hạn • Nhanh tay</p>
      </div>
    </div>

    <div class="cards">
      <article v-for="p in products.slice(8, 12)" :key="'outlet-' + p.id" class="card">
        <div class="thumb">
          <img :src="p.img" alt="" />
          <div class="overlay">
            <button class="overlay-btn" @click="$router.push('/product/' + p.id)">
  ...
  <div class="container">
    <div class="split">
      ...
      <div class="panel">
        ...
        <p>Ghé thăm 1 trong 15 chi nhánh của chúng tôi:</p>
        <div style="display:grid; gap:6px; margin-top:8px">
          <a href="#" @click.prevent="toast('Demo: Hà Nội')">📍 Hà Nội (Cầu Giấy)</a>
          <a href="#" @click.prevent="toast('Demo: Hồ Chí Minh')">📍 Hồ Chí Minh (Quận 1)</a>
          <a href="#" @click.prevent="toast('Demo: Đà Nẵng')">📍 Đà Nẵng</a>
          <a href="#" @click.prevent="toast('Demo: Xem tất cả')" style="color:var(--accent)">→ Xem tất cả 15 chi nhánh</a>
        </div>
      </div>

      <div class="panel">
        <h3 id="member">💎 Membership</h3>
        <p>Đăng ký thành viên & nhận ưu đãi:</p>
        <div style="display:grid; gap:8px; margin-top:12px">
          <div class="form">
            <input v-model="email" placeholder="Nhập email..." />
            <button class="btn primary" @click="subscribe">Đăng ký</button>
          </div>
          <small style="color:var(--muted)">
            ✓ Nhận voucher và thông tin khuyến mại trước tiên
          </small>
        </div>
      </div>
...
DirtyWave là shop áo khoác dành cho những ai thích phong cách tối giản nhưng vẫn có điểm nhấn. Bọn mình tập trung vào form dễ mặc, chất liệu bền và đường may chắc chắn để anh em có thể phối đi làm, đi chơi hay đi phượt đều ổn. Bộ sưu tập trải từ bomber, hoodie, varsity đến áo khoác gió, ưu tiên các tông màu dễ phối và thiết kế gọn gàng, không rườm rà.
      ...
      <div>
        ...
        <div class="img-row">
          <img src="https://upload.wikimedia.org/wikipedia/commons/f/fe/MoMo_Logo.png" height="30">
          <img src="https://upload.wikimedia.org/wikipedia/commons/6/6b/VNPAY-QR.png" height="30">
        </div>
      </div>
...
<div v-if="selectedProduct" class="modal">
  <div class="modal-content">
    ...
    <p style="color:var(--muted)">{{ selectedProduct.cat }}</p>
    <p><b>{{ VND(selectedProduct.price) }}</b></p>

    <button class="btn primary" @click="addToCart(selectedProduct.id); closeModal()">
      Thêm giỏ
    </button>
  </div>
</div>
