...
// Save products to localStorage on init
...
const addToCart = (id) => {
  ...
  localStorage.setItem("cart", JSON.stringify(cart.value))  // Save to localStorage
...
const checkNotifications = () => {
  ...
  if (unread.length > 0) {
    ...
    // Đánh dấu đã đọc
    notifs.forEach(n => n.daDoc = true)
    localStorage.setItem("customerNotifications", JSON.stringify(notifs))
  }
}
...
<button class="btn primary checkout" @click="goCheckout">
Thanh toán
</button>
...
<div v-if="showNotification" class="notification-overlay" @click="closeNotification">
  <div class="notification-box" @click.stop>
    ...
    <div class="notification-content">
      <div v-for="notif in notifications" :key="notif.orderId" class="notification-item">
        <p class="notif-message">{{ notif.message }}</p>
        <small class="notif-time">{{ new Date(notif.ngayTao).toLocaleString('vi-VN') }}</small>
      </div>
    <button class="btn-notif" @click="closeNotification">Đóng</button>
  </div>
