...
const avatarFallback = computed(() => {
  const name = String(displayName.value || 'NV').trim()
...
const resolveLoggedInAccount = async () => {
  ...
  throw new Error('Không tìm thấy tài khoản EMPLOYEE')
}
...
const ensureRealEmployeeName = async () => {
...
const loadProfile = async () => {
  ...
  try {
    ...
    if (!nhanVien.value?.id) {
      throw new Error('Khong tim thay ho so nhan vien gan voi tai khoan nay')
    }

    await ensureRealEmployeeName()
    fillForm()
  } catch (e) {
    console.error('Load employee profile failed:', e)
    error.value = 'Khong the tai ho so nhan vien. Vui long thu lai.'
  } finally {
    loading.value = false
  }
}
...
const saveProfileInfo = async () => {
  ...
  try {
    ...
    console.error('Save employee profile failed:', e)
...
const changePassword = async () => {
  ...
  try {
    ...
    console.error('Change employee password failed:', e)
    ...
    <div class="account-wrap">
      <header class="account-header">
        <div>
          ...
          <p>Cập nhật thông tin, avatar và bảo mật cá nhân</p>
        </div>
      </header>
      ...
      <section class="card" v-if="loading">Đang tải hồ sơ nhân viên...</section>
      ...
      <template v-if="!loading && !error">
        ...
        <section class="card" v-show="activeTab === 'security'">
          ...
          <div class="status-row">
            ...
            <span class="status role">{{ account?.vaiTro || 'EMPLOYEE' }}</span>
